    ...
    class BattleSkill:
        ...
        def OwnerCharCanPaySkillCost(self):
            ...
            if self.Cost_Energy > 0:
                if getattr(self.Owner_BattleChar, "Energy", 0) < self.Cost_Energy:
                    return False

            # mana cost
            if self.Cost_Mana > 0:
                if getattr(self.Owner_BattleChar, "Mana", 0) < self.Cost_Mana:
                    return False

            # current hp % cost
            if self.Cost_PercHealthCurr > 0.0:
                if getattr(self.Owner_BattleChar, "Health", 1) <= 1:
                    return False

            # max hp % cost
            if self.Cost_PercHealthMax > 0.0:
                IntHpCostValue = int(getattr(self.Owner_BattleChar, "HealthMax", 0) * self.Cost_PercHealthMax)
                if getattr(self.Owner_BattleChar, "Health", 0) <= IntHpCostValue:
                    return False
