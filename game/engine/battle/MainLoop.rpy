...
init python:
    ...
    # Barati card minigame isn't implemented yet, so it's never active
        ...
        for Char in BattleScene.BattleChars[0]:
            ...
            if CharID in worldChars:
                if CharInParty(CharID):
                    CharHealth = getattr(Char, "Health", None)
                    CharHealthMax = getattr(Char, "HealthMax", None)
                    if CharHealth is not None and CharHealthMax:
                        HealthRatio = CharHealth / CharHealthMax
