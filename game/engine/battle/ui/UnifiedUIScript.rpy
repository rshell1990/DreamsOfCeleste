init python:
    def Battle_GetTransformSkill(sel_left):
        """Returns the transform/untransform skill the selected char may currently use, or None."""
        if not sel_left:
            return None
        char_ref = getattr(sel_left, "CharRef", {})
        if not char_ref.get("HasAltForm", False):
            return None
        if not getattr(store.BattleScene, "CanTransform", False):
            return None
        if char_ref.get("Transformed", False):
            skill = getattr(sel_left, "Skill_ExtraUnTransform", None)
        elif char_ref.get("AltForm_Unlocked", False):
            skill = getattr(sel_left, "Skill_ExtraTransform", None)
        else:
            skill = None
        return skill if skill else None

    _Battle_WarnedMissingItemActions = set()

    def Battle_MenuHotkey(Index):
        """Returns (keysym, label) for the numeric hotkey of a menu entry, or (None, None) past index 9."""
        if Index < 9:
            return ("K_%d" % (Index + 1), str(Index + 1))
        elif Index == 9:
            return ("K_0", "0")
        return (None, None)

    def Battle_GetItemActionFunc(item_def, ItemID):
        """Looks up the action factory for an item's on_use_battle handler id.
        Warns (once per handler id) when the id is set but not registered in ItemActionLib."""
        on_use = item_def.get("on_use_battle")
        if on_use is None:
            return None
        action_func = store.ItemActionLib.get(on_use) if hasattr(store, "ItemActionLib") else None
        if action_func is None and on_use not in _Battle_WarnedMissingItemActions:
            _Battle_WarnedMissingItemActions.add(on_use)
            renpy.log("WARNING: item '%s' uses on_use_battle handler '%s' which has no entry in ItemActionLib." % (ItemID, on_use))
        return action_func

    def Battle_GetUsableBattleItems():
        """Returns the list of item IDs in the player inventory that have a registered battle-use handler."""
        inv = getattr(store, "player_inv", {})
        items_dict = getattr(store, "all_items", {})
        return [ItemID for ItemID in inv.keys() if items_dict.get(ItemID, {}).get("on_use_battle") is not None]
    ...
    hbox:
        ...
        if sel_left:
            vbox:
                ...
                $ transform_skill = Battle_GetTransformSkill(sel_left)
                if transform_skill:
                    use Battle_SkillIcon(transform_skill, HotKey = "K_x", HotKeyText = "x", FromSkillMenu = False, WaitForPlayerInput = WaitForPlayerInput)

            vbox:
                if can_use_items:
                    $ valid_item_count = len(Battle_GetUsableBattleItems())
        ...
        vbox:
            vpgrid:
                ...
                for SkillIndex, SkillInstance in enumerate(getattr(BattleChar, "Skills", [])):
                    $ skill_keysym, skill_keytext = Battle_MenuHotkey(SkillIndex)
                    if skill_keysym:
                        use Battle_SkillIcon(SkillInstance, HotKey = skill_keysym, HotKeyText = skill_keytext, FromSkillMenu = True, WaitForPlayerInput = WaitForPlayerInput)
                    else:
...
screen Battle_ItemMenu(BattleChar):
    ...
    $ inv = getattr(store, "player_inv", {})
    $ items_dict = getattr(store, "all_items", {})
    $ valid_items = Battle_GetUsableBattleItems()
        ...
        vbox:
            vpgrid:
                ...
                for ItemNum, ItemID in enumerate(valid_items):
                    ...
                    $ action_func = Battle_GetItemActionFunc(item_def, ItemID)
                    $ item_action = action_func(Owner_BattleChar = BattleChar, ItemID = ItemID) if action_func else None
                    $ item_is_usable = inv.get(ItemID, 0) >= 1 and item_action is not None
                    $ item_keysym, _item_keytext = Battle_MenuHotkey(ItemNum)
                    fixed:
                        fit_first True
                        imagebutton:
                            ...
                            sensitive   BattleChar in getattr(store.BattleScene, "ActiveCharsList", []) and item_is_usable
