init python:
    # returns current Madness val
    def InfGetValue():
        return MadnessModule().CurrentValue

    # Adjusts Madness value and performs relevant checks
    def InfChangeBy(ChangeByRaw, silent = False, cantLose = False, SetTo = False):
        InfModule = MadnessModule()
        ...
        ChangeBy = round(ChangeByRaw * DIFFICULTY.Madness_GAIN[CurrentDifficulty])
        if SetTo:
        ...
        if not silent and OriginalValue != InfModule.CurrentValue:
            if InfModule.CurrentValue > OriginalValue:
                AddNotif(tra(_("Madness grows up to ")) + str(int(InfModule.CurrentValue)), Kind = "Madness_raise")
            if InfModule.CurrentValue < OriginalValue:
                AddNotif(tra(_("Madness drops to ")) + str(int(InfModule.CurrentValue)), Kind = "Madness_decrease")

        if InfModule.CurrentValue >= 100 and not cantLose:
            renpy.jump("defeat_generic")
        return

    # generic function to lower Madness when having sex
    def ReduceMadnessFromSex(CharacterID = None, By = None):
        if _in_replay:
            return
        ### if Madness module is inactive, bail
        if MadnessModule().isActive == False:
            return
        ...
        if CharacterID is not None:
            ...
            if len(InfModule.SexedTodayCharacterIDs) > InfModule.UniqueDailyChars:
                AddNotif(tra(_("Your mind seems to be stable for today: your Madness rate remains unchanged.")), Kind = "madness_satisfied")
                return

            ### see if had sex with this char, bail if
            if CharacterID in InfModule.SexedTodayCharacterIDs:
                AddNotif(tra(_("You seem to have lost interest in this 'mate' for today: your Madness rate remains unchanged.")), Kind = "parasite_lostinterest")
                return
    ...
    def InfGainDaily(TrueOrFalse):
        MadnessModule().DailyGain = TrueOrFalse
        return
...
#### this controls Madness value AND stores chars you had sex with for Madness reduction calc
    @AppendToAllQuests
    class MadnessModule(LogicModule):
        def __init__(self):
            super().__init__()
