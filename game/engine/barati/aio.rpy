...
init python:
    Barati_Players = {}
    # per-player limits used by the bet popup
    # name: display name | min_bet / max_bet: gold limits | games_max: games per day
    # games_played_today: counter | daily_reset: day counter the counter was last reset on
    ...
    def Barati_GetPlayerData(OpponentID):
        # fall back to the dummy entry until real opponents are defined
        return Barati_Players.get(OpponentID, Barati_Players["someplayer"])

    def Barati_GetBetValues(PlayerData):
        # derive bet buttons from min/max bet
        BetValues = []
        BetVal = PlayerData["min_bet"]
        while BetVal < PlayerData["max_bet"]:
            BetValues.append(BetVal)
            BetVal *= 2
        BetValues.append(PlayerData["max_bet"])
        return BetValues
...
label barati_game_start:
    ...
    $ BaratiGame.Round = BaratiRoundState(BaratiGame)
...
label barati_round_start:
    ...
    ### dynamic per-round vars: fresh state object for this round
    $ BaratiGame.Round = BaratiRoundState(BaratiGame)
    ...
    while Barati_KeepRoundGoing():
        ...
        while BaratiGame.ActiveSide < 2:
            call barati_side_turn from _call_barati_side_turn
...
# handles one side's turn: either player input (side 0) or AI (side 1)
label barati_side_turn:
...
init python:
    ...
    class BaratiRoundState:
        # holds all per-round mutable state; created fresh at each round start
        def __init__(self, Game):
            self.ActiveSide = 0
            self.PlayersCards = {0:{X:[] for X in range(Game.CardRows)}, 1:{X:[] for X in range(Game.CardRows)}}
            self.CurrentDeck = Game.BaseDeck.copy()
            self.StandingRows = {X:{Y:False for Y in range(Game.CardRows)} for X in range(2)}
    ...
    class BaratiGameClass:
        def __init__(self, OpponentID, BetGoldAmt):
            ...
            # per-round state is held in a BaratiRoundState instance (delegated below)
            self.Round = BaratiRoundState(self)
        ...
        # per-round state delegation so existing code keeps working
        @property
        def ActiveSide(self):
            return self.Round.ActiveSide
        @ActiveSide.setter
        def ActiveSide(self, Value):
            self.Round.ActiveSide = Value
        @property
        def PlayersCards(self):
            return self.Round.PlayersCards
        @property
        def CurrentDeck(self):
            return self.Round.CurrentDeck
        @property
        def StandingRows(self):
            return self.Round.StandingRows
        @property
        def SideHadNoActions(self):
            return self.Round.SideHadNoActions
        @property
        def RoundWinner(self):
            return self.Round.RoundWinner
        @RoundWinner.setter
        def RoundWinner(self, Value):
            self.Round.RoundWinner = Value
    ...
    def Barati_PayOrLoseVictoryGold():
        if store.BaratiGame.BetGoldAmt == None:
            return
        if BaratiGame.GameWinner == 0:
            renpy.say(who = None, what = "you won %s (not really yet)" % store.BaratiGame.BetGoldAmt)
        elif BaratiGame.GameWinner == 1:
            renpy.say(who = None, what = "you lost %s (not really yet)" % store.BaratiGame.BetGoldAmt)
        return
    ...
    def Barati_DoAITurn(Side = 0):
        ...
        OpposingScore = Barati_GetRawRowScore(OpposingSide, RowToActUpon)

        # if enemy stands with a lower score, dont hit (it's unnecessary to act)
        if BaratiGame.StandingRows[OpposingSide][RowToActUpon] and OpposingScore < RowScore:
            Doubt = 1.0
        else:
    ...
    def Barati_EvaluateRoundResults():
        ...
        for PlayerIndex in range(2):
            ...
            for RowIndex in range(BaratiGame.CardRows):
                ThisRowScore = RowScores[PlayerIndex][RowIndex]
                OpposingRowScore = RowScores[OpponentIndex][RowIndex]
    ...
    def Barati_SideCanAct(Side):
        ...
        # for each row we act on: no row of ours is standing/busted and opponent has
        # no rows we could win by hitting or standing on -> nothing to do
        WonNonStandingRows = 0
        for RowIndex in range(BaratiGame.CardRows):
            if BaratiGame.StandingRows[Side][RowIndex]:
                continue
            if Barati_CheckWhetherRowIsBusted(Side, RowIndex):
                continue
    ...
    def IsPlayerInBaratiGame():
        if store.BaratiGame is not None:
            return True
        return False
...
screen Barati_BetPopup(OpponentID):
    default PlayerData = Barati_GetPlayerData(OpponentID)
    default ChosenBet = PlayerData["min_bet"]
    frame:
        align (0.5, 0.5)
        vbox:
            align (0.5, 0.5)
            label PlayerData["name"]
            text "games they can play today: %s" % (PlayerData["games_max"] - PlayerData["games_played_today"])
            text "min bet: %s" % PlayerData["min_bet"]
            text "max bet: %s" % PlayerData["max_bet"]
            text "your bet: %s" % ChosenBet
            hbox:
                for BetVal in Barati_GetBetValues(PlayerData):
                    if BetVal <= PlayerData["max_bet"]:
