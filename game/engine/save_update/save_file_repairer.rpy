...
###### for all saves prior to 0.162
    if SaveGameVersion < "0.162" or DEBUG_ForceSaveUpdate:
        ...
        if not hasattr(store, "DraxCraftRecipesAndIngredientsContainer"):
            $ store.DraxCraftRecipesAndIngredientsContainer = {}
        ...
        if hasattr(store, "current_node_loc"):
            $ del store.allow_quit_navigation_menu
            $ del store.auto_navigation_until
            $ del store.stop_auto_navigating
            $ del store.selected_node
            $ del store.current_route_random_locs
            $ del store.current_node_loc

            if store.current_route is not None:
                # fingers crossed
...
###### 0.164 to 0.165
    if SaveGameVersion == "0.164":
        ...
        if not hasattr(store, "shop_hamun_smithy"):
            $ store.shop_hamun_smithy = {"discount":0.0, "items":{}}

        # beshar crafting
        if not hasattr(store, "HamunSmithyCraftRecipesAndIngredientsContainer"):
            $ store.HamunSmithyCraftRecipesAndIngredientsContainer = {}
...
### yes we could just set it to conf. ver but that way we get clear control of save update sequence
    ...
    if SaveGameVersion == "0.200":
        $ SaveGameVersion = "0.201"
